...
class Kiss{
	...
	function getPostsByIds($ids,$start=0,$count=10){
		$sql="select rowid,title,tag,description,posttime,postname,imgids from post where rowid in ({$ids}) and posttime<{$this->currentTime} order by posttime desc limit {$start},{$count}";
	...
	function getPostsByTime($count=10,$start=0){
		$sql="select rowid,title,tag,description,posttime,postname,imgids from post where posttime<{$this->currentTime} order by posttime desc limit {$start},{$count}";
		return $this->getData($sql);
	}
	...
	*/
	function getPostsByNeighbor2($id){
		...
		if($postTime){
			$sql="select rowid,title,tag,description,postname,imgids from post where posttime<{$postTime} order by rowid desc Limit 1";
			$ret["pre"]=$this->getLine($sql);
			$sql="select rowid,title,tag,description,postname,imgids from post where  posttime>{$postTime} Limit 1";
	...
	function getPostsByNeighbor($id){
		$ret=array();
		$id=intval($id);
		$sql="select rowid,title,tag,description,postname,imgids from post where posttime<{$this->currentTime} and rowid=".($id-1);
		$ret["pre"]=$this->getLine($sql);
		$sql="select rowid,title,tag,description,postname,imgids from post where  posttime<{$this->currentTime} and rowid=".($id+1);
		$ret["next"]=$this->getLine($sql);
		return $ret;
	...
	function getPostsByRand($count=10){
		...
		return $this->getData("select rowid,title,postname,imgids from post where rowid in (".substr($s_ids,1).") and posttime<{$this->currentTime}");
	}
	...
	function hrefFormat($post){
		return $post["rowid"]."_".$post["postname"]."/";
	}
